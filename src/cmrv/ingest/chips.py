...
def make_split(
    ...
    *,
    ...
    if out_prefix:
        ...
        # `source` rides along so train-head can score each source separately. Every
        # adapter writes obs_id as "<source>:...", so this needs no store read. It
        # matters because a distilled source (NIAPS) scored against itself only
        ...
        cols = ["obs_id", "fold", "source"] + (
            ["class_id"] if "class_id" in manifest.columns else []
        )
