    ...
    *,
    ...
    for obs_id, g in man.groupby("obs_id"):
        ...
        if all(mo in by_month for mo in months):
            recs.append((obs_id, int(g["block_id"].iloc[0]), [by_month[mo] for mo in months]))
    if not recs:
    ...
    for i in range(0, len(recs), batch):
        ...
        stacks = np.stack([_load_stack(uris, scale) for _, _, uris in chunk])
    ...
    ds = xr.Dataset(
        {"emb": (("obs", "feat"), emb)},
        coords={"obs_id": ("obs", np.array(obs_ids)), "block_id": ("obs", np.array(block_ids))},
    )
